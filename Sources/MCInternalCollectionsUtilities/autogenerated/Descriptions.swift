...
// the current best way to do this is to duplicate all definitions.
    ...
    @usableFromInline
    func _addressString(for pointer: UnsafeRawPointer) -> String {
        let address = UInt(bitPattern: pointer)
        return "0x\(String(address, radix: 16))"
    }

    @usableFromInline
    func _addressString(for object: AnyObject) -> String {
        _addressString(for: Unmanaged.passUnretained(object).toOpaque())
    }

    @usableFromInline
    func _addressString<T: AnyObject>(for object: Unmanaged<T>) -> String {
        _addressString(for: object.toOpaque())
    }

    @inlinable
    func _arrayDescription<C: Collection>(
        for elements: C
    ) -> String {
        var result = "["
        var first = true
    ...
    @inlinable
    func _dictionaryDescription<Key, Value, C: Collection>(
        for elements: C
