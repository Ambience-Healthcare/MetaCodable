...
// the current best way to do this is to duplicate all definitions.
    ...
    struct _UnsafeBitSet {
        ...
        @usableFromInline
        let _words: UnsafeBufferPointer<_Word>
        ...
        @inlinable
        @inline(__always)
        var _mutableWords: UnsafeMutableBufferPointer<_Word> {
            ensureMutable()
    ...
    extension _UnsafeBitSet {
        ...
        static func _withTemporaryUninitializedBitSet(
            wordCount: Int,
            run body: (inout Self) throws -> Void
        ) rethrows {
            assert(wordCount >= 0)
    ...
    extension _UnsafeBitSet {
        ...
        static func wordCount(forCapacity capacity: UInt) -> Int {
        ...
        @inlinable @inline(__always)
        func isWithinBounds(_ element: UInt) -> Bool {
            element < capacity
        }
        ...
        @usableFromInline
        @discardableResult
        mutating func insert(_ element: UInt) -> Bool {
            ensureMutable()
        ...
        @usableFromInline
        @discardableResult
        mutating func remove(_ element: UInt) -> Bool {
            ensureMutable()
            let index = Index(element)
        ...
        @_effects(releasenone)
        @usableFromInline
        mutating func update(_ member: UInt, to newValue: Bool) -> Bool {
            ensureMutable()
        ...
        @_effects(releasenone)
        @usableFromInline
        mutating func insertAll(upTo max: UInt) {
            assert(max <= capacity)
        ...
        @usableFromInline
        @inline(__always)
        mutating func insertAll(upTo max: Int) {
            precondition(max >= 0)
    ...
    extension _UnsafeBitSet: Sequence {
        ...
        struct Iterator: IteratorProtocol {
            ...
            @inlinable
            init(_ bitset: _UnsafeBitSet) {
                _bitset = bitset
                _index = 0
                _word = bitset.wordCount > 0 ? bitset._words[0] : .empty
            }
