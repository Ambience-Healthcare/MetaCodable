...
public struct OrderedSet<Element> where Element: Hashable {
    @usableFromInline
    typealias _Bucket = _HashTable.Bucket

    @usableFromInline
    var __storage: _HashTable.Storage?
    ...
    init(
        ...
        __storage = table?._storage
        _elements = _uniqueElements
    }

    @inlinable
    @inline(__always)
    var _table: _HashTable? {
...
extension OrderedSet {
    @inlinable
    mutating func _regenerateHashTable(scale: Int, reservedScale: Int) {
    ...
    @inlinable
    mutating func _regenerateHashTable() {
    ...
    @inlinable
    mutating func _regenerateExistingHashTable() {
...
extension OrderedSet {
    @inlinable
    func _find(_ item: Element) -> (index: Int?, bucket: _Bucket) {
        _find_inlined(item)
    }

    @inlinable
    @inline(__always)
    func _find_inlined(_ item: Element) -> (index: Int?, bucket: _Bucket) {
    ...
    @inlinable
    func _bucket(for index: Int) -> _Bucket {
...
extension OrderedSet {
    @inlinable
    @inline(never)
    __consuming func _extractSubset(
        ...
        if c == self.count {
            if extraCapacity <= _capacity - self.count {
                return self
            }
            var copy = self
...
extension OrderedSet {
    @inlinable
    @discardableResult
    mutating func _removeExistingMember(
        at index: Int,
        in bucket: _Bucket
    ) -> Element {
...
public extension OrderedSet {
    ...
    func filter(
        ...
        try _UnsafeBitSet.withTemporaryBitSet(capacity: count) { bitset in
