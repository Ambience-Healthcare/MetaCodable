...
@usableFromInline
typealias _UnsafeHashTable = _HashTable.UnsafeHandle
...
extension _HashTable {
    ...
    struct UnsafeHandle {
        ...
        @usableFromInline
        var _header: UnsafeMutablePointer<Header>

        /// A pointer to bucket storage.
        @usableFromInline
        var _buckets: UnsafeMutablePointer<UInt64>
...
extension _HashTable.UnsafeHandle {
    ...
    @inlinable
    @inline(__always)
    var scale: Int { _header.pointee.scale }
    ...
    @inlinable
    @inline(__always)
    var reservedScale: Int { _header.pointee.reservedScale }

    /// The hasher seed to use within this hash table.
    @inlinable
    @inline(__always)
    var seed: Int { _header.pointee.seed }
    ...
    @inlinable
    func position(of bucket: Bucket) -> (word: Int, bit: Int) {
...
extension _UnsafeHashTable {
    @inlinable
    func _find<Base: RandomAccessCollection>(
        _ item: Base.Element,
        in elements: Base
...
extension _UnsafeHashTable {
    @usableFromInline
    func firstOccupiedBucketInChain(with bucket: Bucket) -> Bucket {
        var bucket = bucket
        repeat {
...
extension _UnsafeHashTable {
    @inlinable
    func adjustContents<Base: RandomAccessCollection>(
