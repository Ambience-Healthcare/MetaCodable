...
extension _HashTable {
    ...
    @usableFromInline
    @inline(__always)
    static var maximumUnhashedCount: Int {
        @_effects(readnone)
        get {
            (1 &<< (minimumScale - 1)) - 1
        }
    ...
    @usableFromInline
    @_effects(readnone)
    static func minimumCapacity(forScale scale: Int) -> Int {
    ...
    @usableFromInline
    @_effects(readnone)
    static func maximumCapacity(forScale scale: Int) -> Int {
    ...
    @usableFromInline
    @_effects(readnone)
    static func scale(forCapacity capacity: Int) -> Int {
