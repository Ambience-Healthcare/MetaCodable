...
extension _HashTable.UnsafeHandle {
    @usableFromInline
    typealias BucketIterator = _HashTable.BucketIterator
    ...
    @inlinable
    @inline(__always)
    func idealBucket<Element: Hashable>(for element: Element) -> Bucket {
    ...
    @inlinable
    @inline(__always)
    func bucketIterator(startingAt bucket: Bucket) -> BucketIterator {
    ...
    func _startIterator(
        ...
        var (word, bit) = position(of: bucket)
        if bit + scale <= 64 {
...
extension _HashTable.BucketIterator {
    ...
    @inlinable
    @inline(__always)
    var currentBucket: Bucket { _currentBucket }
