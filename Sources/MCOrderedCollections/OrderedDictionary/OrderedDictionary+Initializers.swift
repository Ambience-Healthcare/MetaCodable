...
public extension OrderedDictionary {
    ...
    init(minimumCapacity: Int, persistent: Bool = false) {
        _keys = OrderedSet(minimumCapacity: minimumCapacity, persistent: persistent)
        _values = []
...
public extension OrderedDictionary {
    ...
    init(
        ...
        if let keysAndValues = _specialize(
            keysAndValues, for: [Key: Value].self
        ) {
...
public extension OrderedDictionary {
    ...
    internal init<S: Sequence>(
        ...
        for value in values {
            ...
            updateValue(forKey: key, default: Value()) { array in
                array.append(value)
            }
        }
    }
...
public extension OrderedDictionary {
    ...
    init(
        ...
        #else
            _keys = .init(uncheckedUniqueElements: keys)
            _values = .init(values)
